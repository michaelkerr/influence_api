...
opt_param_list = ['subforum',
		...
		'twit_collect',
		'type']
format_param_list = ['return_csv', 'return_graph', 'format']
...
def create_filename(file_params):
	...
	if ('twit_collect' in file_params.keys()) and (file_params['twit_collect'] is not None):
		filename += '_' + file_params['twit_collect'].replace(' ', '_').replace('_track', '')
	filename += '.graphml'
	return filename
...
def centrality():
	...
	## >If CSV requested
	if for_params['return_csv'] is not None:
		if for_params['return_csv'].lower() == 'true':
			influence_name = create_filename(params).replace('.graphml', '.txt')
			influence_final = ''
			if '>calc_error<' not in calc_metric.keys():
				for item in calc_metric.items():
					influence_final += item[0].encode('utf-8') + ', ' + str(item[1]) + '\n'
			## >Create the appropriate response to return the graphml
			response = make_response(influence_final)
			response.headers["Content-Type"] = 'text/txt'
			response.headers["Content-Disposition"] = 'attachment; filename=' + influence_name
			return response
